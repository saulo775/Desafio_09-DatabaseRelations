...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    const custormerExists = await this.customersRepository.findById(
      customer_id,
    );
    ...
    if (!existentProducts.length) {
      throw new AppError('Could not find any products withe the given ids');
    }
